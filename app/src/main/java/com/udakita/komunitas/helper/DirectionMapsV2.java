...
import static com.udakita.komunitas.helper.MyFunction.timeConversion;
...
public class DirectionMapsV2 {
	...
	private List<LatLng> listpolylineBefore;
	...
	public void gambarRoute(GoogleMap map, String dataPoly) {
		...
		for (int i = 0; i < polyz.size() - 1; i++) {
			...
			Polyline line = map.addPolyline(new PolylineOptions()
					...
					.color(Color.parseColor("#00FFFF")).geodesic(true));
	...
	public void gambarRoutePOI(GoogleMap mMap, String point) {

		/*//apakah ada isi dari polyline sebelumnya
		if (polylineBefore!=null && mMapBefore!=null)
		{
			listpolylineBefore  = decodePoly(polylineBefore);
			for (int i = 0; i < listpolylineBefore.size() - 1; i++) {
				LatLng srcBefore = listpolylineBefore.get(i);
				LatLng destBefore = listpolylineBefore.get(i + 1);
				polylines.add(mMap.addPolyline(new PolylineOptions()
						.add(new LatLng(srcBefore.latitude, srcBefore.longitude),
								new LatLng(destBefore.latitude, destBefore.longitude)).width(0)
						.color(Color.DKGRAY)));
			}

			//To Clear Polyline Before
			if (polylines.size()> 0){
		...
		//add/draw current polyline
        //HeroHelper.pesan(context,point);
		polyz = decodePoly(point);
		for (int i = 0; i < polyz.size() - 1; i++) {
			LatLng src = polyz.get(i);
			LatLng dest = polyz.get(i + 1);
			polylines.add(mMap.addPolyline(new PolylineOptions()
		...
		try {
			...
			if (step.length() > 0) {
				...
				for (int i = 0; i < step.length(); i++) {
					...
					data.put("durasiTotal", timeConversion(durasiTotal));
					datax.add(data);
